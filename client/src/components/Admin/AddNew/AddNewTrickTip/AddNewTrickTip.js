...
const AddNewTrickTip = ({ addNew }) => {
  ...
  useEffect(()=>{
    ...
    // Adjust scroll speed by multiplying the deltaY value
    textarea.addEventListener("wheel", (event) => {
      event.preventDefault() // Prevent default scroll behavior
      textarea.scrollTop += event.deltaY * 0.05 // Adjust scroll speed by changing the multiplier
    })
  })
  ...
  const submitTrickTip = (trickTip) => {
    postNewTrickTip(trickTip).then((result) => {
  ...
  return (
    <div className={Styles.wrapper}>
      ...
      <div>
        <img
          src={trickTipInputs.videoLink ? uploadedVideoImg : newTrickTipImg}
          alt="Skater"
        />
